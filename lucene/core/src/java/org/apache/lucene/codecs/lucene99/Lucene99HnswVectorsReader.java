...
public final class Lucene99HnswVectorsReader extends KnnVectorsReader
  ...
  private void search(
    ...
    // Per-query QueryAccessHint.POINT consumer: when the collector advertises POINT, wrap the
    // scorer so each batch of neighbor vectors is prefetched before bulkScore() computes their
    // distances (see PrefetchableFlatVectorScorer#wrapWithPrefetch for the no-op fall-through
    // cases). The wrap is skipped entirely when the hint is absent so default behaviour is
    // unchanged.
